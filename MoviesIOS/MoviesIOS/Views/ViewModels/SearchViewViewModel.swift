...
class MovieSearchViewViewModel {
    ...
    init(query: Driver<String>, moviesApiClient: MoviesAPIClient) {
    ...
    private func searchMovie(query: String?) {
        ...
        moviesApiClient.searchMovie(query: query, params: nil, successHandler: {[weak self] (response) in
            ...
            self?._movies.accept(Array(response.movies.prefix(5)))
        }) { [weak self] (error) in
            self?._isFetching.accept(false)
